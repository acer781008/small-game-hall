const GAME_IDS=['bingo','sudoku','memory','shelf','lianliankan','puzzle','numberbattle','turtle'];
module.exports = async function createActivityStore({ adapter, file }) {
  ...
  function list(){return Object.entries(data.activities||{}).map(([code,a])=>normalize({...a,code})).sort((a,b)=>(Number(b.createdAt)||0)-(Number(a.createdAt)||0)).map(a=>JSON.parse(JSON.stringify(a)));}

  return {GAME_IDS,current,currentCode,create,update,get,list,isCurrent,status,isOpen,isGameEnabled,recordRetentionCandidates,markRecordsCleared};
};
