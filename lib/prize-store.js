module.exports = async function createPrizeStore({ adapter, file, getCurrentActivityCode }) {
  ...
  const fresh=()=>({awardMode:'A',maxClaimsPerPlayer:1,prizes:[],records:[],cPrizes:[],cRecords:[],claims:{}});
  ...
  function state(c){
    ...
    return {
      activityCode:code(c),awardMode:b.awardMode,maxClaimsPerPlayer:Number(b.maxClaimsPerPlayer)||0,
  ...
  function publicState(c){
    const s=state(c);
    return {
      activityCode:s.activityCode,awardMode:s.awardMode,
  ...
  async function setSettings(patch={},c){
    ...
    return run(root=>{
      ...
      return {
        activityCode:cc,awardMode:b.awardMode,maxClaimsPerPlayer:b.maxClaimsPerPlayer,
        prizes:b.prizes.map(cleanPrize),records:b.records.slice().reverse(),
        cPrizes:b.cPrizes.map(cleanCPrize),cRecords:b.cRecords.slice().reverse()
      };
    });
  ...
  return {state,publicState,setSettings,setMaxClaims,addPrize,updatePrize,removePrize,addCPrize,updateCPrize,removeCPrize,award};
};
