module.exports = async function createPrizeStore({ adapter, file, getCurrentActivityCode }) {
  const GAME_IDS=['bingo','sudoku','memory','shelf','lianliankan','puzzle'];
  const fresh=()=>({awardMode:'A',maxClaimsPerPlayer:1,prizes:[],records:[],claims:{}});
  ...
  function bucket(root,c){
    ...
    b.awardMode=String(b.awardMode||'A').toUpperCase()==='B'?'B':'A';
  ...
  function state(c){
    ...
    return {activityCode:code(c),awardMode:b.awardMode,maxClaimsPerPlayer:Number(b.maxClaimsPerPlayer)||0,prizes:b.prizes.map(cleanPrize),records:b.records.slice().reverse()};
  }
  function publicState(c){
    const s=state(c);
    return {activityCode:s.activityCode,awardMode:s.awardMode,prizes:s.prizes.filter(p=>p.enabled!==false)};
  }
  ...
  async function setSettings(patch={},c){
    ...
    return run(root=>{
      ...
      if(patch.awardMode!==undefined)b.awardMode=String(patch.awardMode||'A').toUpperCase()==='B'?'B':'A';
      return {activityCode:cc,awardMode:b.awardMode,maxClaimsPerPlayer:b.maxClaimsPerPlayer,prizes:b.prizes.map(cleanPrize),records:b.records.slice().reverse()};
    });
  }
  ...
  function normalizeGameId(gameId,game){
    ...
    return ({'賓果':'bingo','數獨':'sudoku','翻牌':'memory','貨架整理':'shelf','連連看':'lianliankan','拼圖挑戰':'puzzle','拼圖':'puzzle'})[n]||'';
  }
  async function award({activityCode,gameId,game,playerName,playerKey,gameRef}){
  ...
  return {state,publicState,setSettings,setMaxClaims,addPrize,updatePrize,removePrize,award};
};
