module.exports = async function createPrizeStore({ adapter, file, getCurrentActivityCode }) {
  ...
  async function clearRecords(c){
    ...
    return run(root=>{
      ...
      b.records=[];
      b.cRecords=[];
      // 只清除可查看的得獎紀錄；獎品庫存與已領獎次數保留，避免重複領獎。
      return {activityCode:cc,cleared:true,normalCount,cCount};
    });
  }
  return {state,publicState,setSettings,setMaxClaims,addPrize,updatePrize,removePrize,addCPrize,updateCPrize,removeCPrize,award,clearRecords};
};
