module.exports = async function createCompletionStore({adapter,file,getCurrentActivityCode}){
  ...
  function gameBucket(root,c,gid,gname=''){
    ...
    if(!b.games[gid])b.games[gid]={name:String(gname||gid||'遊戲'),players:{},seen:{}};
    const g=b.games[gid];g.name=String(gname||g.name||gid||'遊戲');g.players=g.players&&typeof g.players==='object'?g.players:{};g.seen=g.seen&&typeof g.seen==='object'?g.seen:{};return g;
  }
  ...
  return {playerStats,leaderboard,allPlayerStats,record};
};
