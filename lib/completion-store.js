module.exports = async function createCompletionStore({adapter,file,getCurrentActivityCode}){
  ...
  async function clearActivity(activityCode){
    const cc=code(activityCode);
    const tx=await adapter.transact('completions',file,rootDefaults,root=>{
      root.activities=root.activities&&typeof root.activities==='object'?root.activities:{};
      delete root.activities[cc];
      return {activityCode:cc,cleared:true};
    });
    data=tx.data;
    return clone(tx.result);
  }
  return {playerStats,leaderboard,scoreLeaderboard,allPlayerStats,record,recordScore,clearActivity};
};
