...
module.exports=function initTurtle({app,awardPrize,recordPlay=async()=>({}),recordCompletion=async()=>({}),recordScore=async()=>({}),getCompletionStatus=()=>({allowed:true,completions:0,max:0}),isGameEnabled,getActivityCode,isActivityOpen=()=>true}){
  ...
  let settings={difficultyMode:'host',difficulty:'normal',timeMode:'limited',minutes:3,targetMode:'limited',target:30};
  const runs=new Map();
  function normalize(s={}){
    ...
    const minutes=Math.max(1,Math.min(999,Math.floor(Number(s.minutes)||Number(settings.minutes)||3)));
    const targetMode=s.targetMode==='unlimited'?'unlimited':'limited';
    const target=Math.max(1,Math.min(99999,Math.floor(Number(s.target)||Number(settings.target)||30)));
    return {difficultyMode,difficulty,timeMode,minutes,targetMode,target};
  }
  ...
  app.post('/games/turtle/api/start',async(req,res)=>{
    ...
    const k=key(activity,player),existing=runs.get(k);if(existing)return res.json({ok:true,...pub(existing),resumed:true});
    ...
    if(settings.difficultyMode==='player'&&DIFFS.includes(req.body?.difficulty))difficulty=req.body.difficulty;
  ...
  app.post('/games/turtle/api/finish',async(req,res)=>{
    ...
    if(activity!==String(getActivityCode()))return res.status(404).json({ok:false,message:'活動不存在'});
    const k=key(activity,player),r=runs.get(k);if(!r||r.runId!==runId)return res.status(404).json({ok:false,message:'找不到本局資料'});
    runs.delete(k);
    ...
    if(completed){
      ...
      prize=await awardPrize({activityCode:activity,gameId:'turtle',game:'烏龜島大作戰',playerName:player,playerKey:player,gameRef:activity,tier:r.tier,selection:{難度:diffLabel,分數:`${score} 分`,目標:`${r.settings.target} 分`},elapsedMs});
    }
