...
const BINGO_READY_DELAY_MS = 3000;
...
function normalizeTierMap(m={}){const d={25:'C1',36:'C2',49:'C3',64:'C3'},out={};for(const k of [25,36,49,64]){const t=String(m[k]||d[k]).toUpperCase();out[k]=['C1','C2','C3'].includes(t)?t:d[k]}return out}
function roomInfo(room){return {roomId:room.id,controlMode:room.controlMode||'admin',version:room.version,theme:room.theme,playMode:room.playMode,items:room.items,size:room.size,phase:room.phase,timeLimitEnabled:room.timeLimitEnabled,gameSeconds:room.gameSeconds,gameMinutes:Number(room.gameSeconds)>0?Number(room.gameSeconds)/60:0,drawIntervalMs:room.drawIntervalMs,targetLines:1,note:room.note,startCount:room.startCount||0,finishCount:room.finishCount||0,selfSelect:!!room.selfSelect,allowedVersions:room.allowedVersions||['farm','number','picture'],allowedSizes:room.allowedSizes||[25,36,49,64],allowedPlayModes:room.allowedPlayModes||['normal','advanced'],tierBySize:room.tierBySize||normalizeTierMap()};}
function makeItems(version,size,playMode){if(version==='picture'){const shuffled=shuffleForRun(MAHJONG_ITEMS);return playMode==='normal'?shuffled.slice(0,size):MAHJONG_ITEMS.slice()}if(version==='farm'){const poolSize=playMode==='normal'?size:({25:45,36:60,49:75,64:90}[size]||45);return pickFarmItems(poolSize)}return null}
function ensureRoom(roomId){roomId=String(roomId||'').trim();if(!roomId)return null;if(rooms.has(roomId))return rooms.get(roomId);const size=25,version='farm',playMode='normal';const room={id:roomId,controlMode:'admin',version,theme:'farm',playMode,items:pickFarmItems(size),size,phase:'playing',timeLimitEnabled:true,gameSeconds:600,drawIntervalMs:4000,targetLines:1,note:'',players:new Map(),startCount:0,finishCount:0,selfSelect:false,allowedVersions:['farm','number','picture'],allowedSizes:[25,36,49,64],allowedPlayModes:['normal','advanced'],tierBySize:normalizeTierMap()};rooms.set(roomId,room);return room;}
function hallGetSettings(roomId){const r=ensureRoom(roomId);return r?roomInfo(r):null;}
function hallSetSettings(roomId,patch={}){const room=ensureRoom(roomId);const sizes=[25,36,49,64],versions=['number','picture','farm'],modes=['normal','advanced'];room.selfSelect=patch.selfSelect===true;room.allowedVersions=Array.isArray(patch.allowedVersions)?patch.allowedVersions.filter(x=>versions.includes(x)):room.allowedVersions;if(!room.allowedVersions.length)room.allowedVersions=[room.version];room.allowedSizes=Array.isArray(patch.allowedSizes)?[...new Set(patch.allowedSizes.map(Number).filter(x=>sizes.includes(x)))]:room.allowedSizes;if(!room.allowedSizes.length)room.allowedSizes=[room.size];room.allowedPlayModes=Array.isArray(patch.allowedPlayModes)?patch.allowedPlayModes.filter(x=>modes.includes(x)):room.allowedPlayModes;if(!room.allowedPlayModes.length)room.allowedPlayModes=[room.playMode];room.tierBySize=normalizeTierMap(patch.tierBySize||room.tierBySize);const size=sizes.includes(Number(patch.size))?Number(patch.size):room.size,version=versions.includes(patch.version)?patch.version:room.version,playMode=modes.includes(patch.playMode)?patch.playMode:room.playMode;if(version==='picture'&&![25,36].includes(size))throw new Error('麻將版目前只開放 5×5、6×6');room.version=version;room.theme=version==='picture'?'mahjong':version==='farm'?'farm':null;room.playMode=playMode;room.size=size;const mins=patch.gameMinutes!==undefined?Number(patch.gameMinutes):null;room.gameSeconds=mins!==null?(mins===0?0:Math.max(60,Math.min(7200,mins*60))):(Number(patch.gameSeconds)===0?0:Math.max(60,Number(patch.gameSeconds||room.gameSeconds||600)));room.drawIntervalMs=Math.max(1000,Number(patch.drawIntervalMs||room.drawIntervalMs||4000));room.items=makeItems(version,size,playMode);io.to(room.id).emit('roomState',{...roomInfo(room),ranking:ranking(room),playerCount:onlinePlayerCount(room)});return roomInfo(room);}
...
function runSourcePool(cfg){if(cfg.version==='farm'||cfg.version==='picture')return (cfg.items||[]).map(x=>x.id);const max=cfg.playMode==='normal'?cfg.size:({25:50,36:60,49:75,64:90}[cfg.size]||cfg.size);return Array.from({length:max},(_,i)=>i+1)}
function normalizeRunBoard(cfg,input){const pool=runSourcePool(cfg),allowed=new Set(pool.map(String)),seen=new Set(),out=[];for(const v of Array.isArray(input)?input:[]){const k=String(v);if(!allowed.has(k)||seen.has(k))continue;seen.add(k);out.push(pool.find(x=>String(x)===k));if(out.length>=cfg.size)break}if(out.length===cfg.size)return out;return shuffleForRun(pool).slice(0,cfg.size)}
...
function publicActiveRun(run){if(!run)return null;return {runId:run.id,startedAt:run.startedAt,endsAt:run.endsAt,size:run.size,version:run.version,playMode:run.playMode,drawIntervalMs:run.drawIntervalMs,boardNumbers:run.boardNumbers||[],drawPool:run.drawPool||[],items:run.items||[],tier:run.tier,selection:run.selection};}
